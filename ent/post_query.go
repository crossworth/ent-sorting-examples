...
func (pq *PostQuery) QueryUsers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, post.UsersTable, post.UsersPrimaryKey...),
		)
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withUsers; query != nil {
		...
		query.Where(func(s *sql.Selector) {
			...
			s.Join(joinT).On(s.C(user.FieldID), joinT.C(post.UsersPrimaryKey[1]))
			s.Where(sql.InValues(joinT.C(post.UsersPrimaryKey[0]), edgeids...))
			columns := s.SelectedColumns()
			s.Select(joinT.C(post.UsersPrimaryKey[0]))
